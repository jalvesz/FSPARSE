...
submodule (fsparse_krylov) fsparse_krylov_COO
    implicit none
    type, extends(abs_op_sp) :: linop_sp
        type(COO_sp) :: ptr_COO_sp
        type(diagonal_sp) :: ptr_pcd_sp
    contains
    ...
    type, extends(abs_op_dp) :: linop_dp
        type(COO_dp) :: ptr_COO_dp
        type(diagonal_dp) :: ptr_pcd_dp
    contains
    ...
    module subroutine cgsolve_COO_sp(A,b,x,di,rtol,maxiter,restart)
        ...
        integer :: maxiter_
        logical :: restart_
        real(sp), allocatable, target :: diag(:)
        !-------------------------
        type(linop_sp) :: op
        !-------------------------
        rtol_    = 1e-8_sp
        ...
        call COO2diagonal(A,diag)
        where(abs(diag)>0._sp) diag = 1._sp/diag
        op%ptr_pcd_sp%data => diag
        !---------------- 
        op%ptr_COO_sp = A
    ...
    module subroutine cgsolve_COO_dp(A,b,x,di,rtol,maxiter,restart)
        ...
        allocate(diag,mold=x)
        call COO2diagonal(A,diag)
        where(abs(diag)>0._dp) diag = 1._dp/diag
        op%ptr_pcd_dp%data => diag
        !---------------- 
        op%ptr_COO_dp = A
